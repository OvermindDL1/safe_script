defmodule SafeScript do
  ...
  def eval_expressions(input, opts \\ []) do
    with\
      {:ok, form} <- compile_expressions(input, opts),
      do: interpret_forms(form, opts)
  end
  ...
  ## Examples
      ...
      iex> {succ, _form} = SafeScript.compile_expressions("2 + 2")
      iex> succ
      :ok
  ...
  def compile_expressions(input, opts \\ []) do
    with\
      {:ok, compiler} <- get_compiler(opts),
      do: compiler.compile_expressions(input, opts)
  end
  ...
  ## Examples
      ...
      iex> {:ok, form} = SafeScript.compile_expressions("2")
      iex> {:ok, {_env, result}} = SafeScript.interpret_forms(form)
      iex> result
      2

  """
  def interpret_forms(form, opts \\ []) do
    with\
      {:ok, compiler} <- get_compiler(opts),
      do: compiler.interpret_forms(form, opts)
  end


  defp get_compiler(opts) do
    case opts[:compiler] do
      nil ->
        case opts[:lang] do
          nil -> {:ok, SafeScript.Lang.Elixir}
          lang when is_atom(lang) or is_binary(lang) ->
            try do
              compiler = Module.safe_concat(SafeScript.Lang, lang)
              if :erlang.function_exported(compiler, :compile_expressions, 2) and :erlang.function_exported(compiler, :interpret_forms, 2) do
                {:ok, compiler}
              else
                {:error, {:unsupported_language, lang, :missing_compiler_at, compiler}}
              end
            catch
              ArgumentError, _ -> {:error, {:unsupported_language, lang}}
            end
        end
      compiler -> compiler
    end
